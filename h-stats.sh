...
stats_raw=`echo '{"id":0,"jsonrpc":"2.0","method":"miner_getstat1"}' | nc -w 5 localhost 5555 | jq '.result'`
if [[ $? -ne 0  || -z $stats_raw ]]; then
	...
	local stats_detail=`echo '{"id":0,"jsonrpc":"2.0","method":"miner_getstatdetail"}' | nc -w 5 localhost 5555 | jq '.result'`
